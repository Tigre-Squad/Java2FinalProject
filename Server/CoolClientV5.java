...
public class CoolClientV5 extends JFrame {
   JTextArea jtaBig;
   JTextField jtfWord;
   JTextField jtfIP;
   ...
   public static void main(String[] args) {
      ...
      CoolClientV5 meow = new CoolClientV5();
      ...
      if (cont) {
         ...
         if (bigCont) {
            ...
            try {
               ...
               buff = new ObjectInputStream(in);
            } catch (IOException eh) {
            }
            Listener listen = new Listener(s, buff);
            listen.start();
   ...
   class Listener extends Thread {
      Socket s;
      ObjectInputStream boof;

      public Listener(Socket ss, ObjectInputStream oinn) {
         s = ss;
         boof = oinn;
      }
      ...
      public void run() {
         ...
         ObjectInputStream buff = boof;

         while (!noconect) {
            String readout = "";
            try {
               Object recive = buff.readObject();
               readout = (String) recive;
            } catch (IOException eh) {
            } catch (ClassNotFoundException nooo) {
            }
